...
void VirtualMachine::run(string filename) {
    ...
    inputFileName = filename.erase(filename.length() -2, 2) + ".o";
    readFileName = filename.erase(inputFileName.length() -2, 2) + ".in";
    writeFileName = filename.erase(readFileName.length() -3, 2) + ".out";
    

    // set variables  
    pc = ir = sr = base = limit = clock = reg[0] = reg[1] = reg[2] = reg[3] = 0;
    sp = 256;
    int currentMemLocation = 0;

    // read into memory, increase limit
    fstream readIntoRD;
    readIntoRD.open(inputFileName.c_str(), ios::in);
	
    if (!readIntoRD.is_open()) {
        cout << inputFileName << " failed to open.\n";
        exit(1);
    }

    string line;
    getline(readIntoRD, line);
    
    while (!readIntoRD.eof()) {
        mem[currentMemLocation++] = atoi(line.c_str());
        getline(readIntoRD, line);
    }

    readIntoRD.close();

    limit = currentMemLocation;
    ...
    fstream output;
    output.open(writeFileName.c_str(), ios::in | ios::out | ios::ate);
    output << "clock = " << clock << endl;
    output.close();
...
{
    if (finalcode.f2.ADDR >= base + limit || finalcode.f2.ADDR < base) {
    ...
    if (finalcode.f2.I == 0) 
        reg[finalcode.f2.RD] = mem[finalcode.f2.ADDR];
    else
        loadi();
}
...
void VirtualMachine::store() //  mem[ADDR] = r[RD]
{
    ...
    mem[finalcode.f2.ADDR] = reg[finalcode.f2.RD];
}
...
void VirtualMachine::read() // read new content of r[RD] from .in file
{
    ...
    fstream readIntoRD;
    readIntoRD.open(readFileName.c_str(), ios::in);
    ...
    int readIn = atoi(line.c_str());

    reg[finalcode.f1.RD] = readIn;    
}
...
void VirtualMachine::write() // write r[RD] into .out file
{
    ...
    fstream output;
    output.open(writeFileName.c_str(), ios::out);
    
    int temp = reg[finalcode.f1.RD] & 0x8000;
    
    if (temp) 
        temp = reg[finalcode.f1.RD] | 0xffff0000;
    else
        temp = reg[finalcode.f1.RD] & 0xffff;
